...
if OS == 'Windows' then
	...
	function fs.chdir(path)
		...
		if C._chdir(path) ~= 0 then return nil, errno() end
		return true
	end
	...
	function fs.curdir()
		...
		while true do
			...
			if C._getcwd(buf, size) ~= nil then
				return ffi.string(buf)
			end
			local err = errno()
			if err ~= 'Result too large' then
				return nil, err
			end
			size = size * 2
		end
	...
	function fs.rmdir(path)
		...
		if C._rmdir(path) == 0 then
			return true
		end
		return nil, errno()
	end

	function fs.touch(path, acct, modtime)
		local buf
		if type(acct) == 'number' then
			modtime = modtime or acct
		...
		ffi.copy(p, path)
		local utime = abi'64bit' and C._utime64 or C._utime32
	...
	function fs.setmode(file, mode)
		...
		local prev_mode = C._setmode(C._fileno(file), mode)
		if prev_mode == -1 then
			return nil, errno()
		end
	...
	local function check_is_dir(path)
		return fs.attr(path, 'mode') == 'directory' and 1 or 0
	end

	function fs.link(old, new)
		local is_dir = check_is_dir(old)
		if C.CreateSymbolicLinkW(
				wchar_t(new),
	...
	if abi'64bit' then
		...
		findfirst = C._findfirst64
		findnext = C._findnext64
	else
		ffi.cdef([[
		...
		findfirst = C._findfirst32
		findnext = C._findnext32
	end

	local function findclose(dentry)
		if dentry and dentry.handle ~= -1 then
			C._findclose(dentry.handle)
			dentry.handle = -1
		end
	end
	...
	local function lock(fh, mode, start, len)
		...
		if not len or len <= 0 then
			if C.fseek(fh, 0, SEEK_END) ~= 0 then
				return nil, errno()
			end
			len = C.ftell(fh)
		end
		...
		if C.fseek(fh, start, SEEK_SET) ~= 0 then
			return nil, errno()
		end
		local fd = C._fileno(fh)
		if C._locking(fd, lkmode, len) == -1 then
			return nil, errno()
		end
		return true
	end

	function fs.lock(filehandle, mode, start, length)
	...
	function fs.unlock(filehandle, start, length)
		if io.type(filehandle) ~= 'file' then
	...
	function fs.mkdir(path, mode)
		if C.mkdir(path, mode or 509) == 0 then
			return true
		end
		return nil, errno()
	end
	...
	function fs.touch(path, acctime, modtime)
		local buf
		if type(acctime) == 'number' then
			modtime = modtime or acctime
	...
	function fs.link(old, new, symlink)
		local f = symlink and C.symlink or C.link
		if f(old, new) == 0 then
			return true
		end
		return nil, errno()
	...
	local dir_obj_type = ffi.metatype([[ struct {
		DIR* _dentry;
		bool closed;
	} ]], {
		__index = { next=iterator, close=close },
		__gc = close,
	})
	...
	local function lock(fd, mode, start, len)
		...
		if C.fcntl(fd, F_SETLK, flock) == -1 then
			return nil, errno()
		end
		return true
	end
...
if OS == 'Windows' then
	...
	function create_lockfile(dir_lock, _, lockname)
		...
		dir_lock.lockname = C.CreateFileW(lockname, GENERIC_WRITE, 0, nil, CREATE_NEW,
				FILE_NORMAL_DELETE_ON_CLOSE, nil)
		return dir_lock.lockname ~= ffi.cast('void*', -1)
	end

	function delete_lockfile(dir_lock)
		return C.CloseHandle(dir_lock.lockname)
	end
else
	...
	function create_lockfile(dir_lock, path, lockname)
		...
		return C.symlink(path, lockname) == 0
	end

	function delete_lockfile(dir_lock)
		return C.unlink(dir_lock.lockname)
	end
end
...
if OS == 'Linux' then
	...
	elseif ARCH == 'x86' then
		...
		stat_syscall_num = abi'64bit' and 106 or 195
		lstat_syscall_num = abi'64bit' and 107 or 196
	...
	elseif ARCH == 'mips' or ARCH == 'mipsel' then
		...
		stat_syscall_num = abi'64bit' and 4106 or 4213
		lstat_syscall_num = abi'64bit' and 4107 or 4214
	end

	if stat_syscall_num then
		stat_func = function(filepath, buf)
			return C.syscall(stat_syscall_num, filepath, buf)
		end
		lstat_func = function(filepath, buf)
			return C.syscall(lstat_syscall_num, filepath, buf)
		end
	else
...
-- Add target field for symlinkattr, which is the absolute path of linked target
...
if OS == 'Windows' then
	...
	function get_link_target_path(link_path)
		...
		while true do
			local buf = ffi.new('char[?]', size)
			local read = C.readlink(link_path, buf, size)
			if read == -1 then
				return nil, errno()
			end
			if read < size then
...
local stat_type = ffi.metatype('stat', {
...
local function attr(filepath, a, followsymlink)
	local buf = ffi.new(stat_type)
	local func = followsymlink and stat_func or lstat_func
	...
	if atype == 'string' then
		local value
		if a == 'target' and not followsymlink then
			value = get_link_target_path(filepath)
		else
			value = buf[a]
		end
		if value == nil then
			error('invalid attribute name \''..a..'\'')
		end
		return value
	else
		local tab = (atype == 'table') and a or {}
		for k, _ in pairs(attr_handlers) do
			tab[k] = buf[k]
		end
		if not followsymlink then
...
function fs.attr(filepath, a)
	return attr(filepath, a, true)
end

function fs.symlinkattr(filepath, a)
	return attr(filepath, a, false)
end
